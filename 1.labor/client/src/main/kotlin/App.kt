...
class App(val canvas : HTMLCanvasElement, val overlay : HTMLDivElement) {
  ...
  fun registerEventHandlers() {
    ...
    canvas.onmousemove = { 
      ...
      if(mouseDown){
        scene.modelMatrix.translate(
                2*(event.clientX.toFloat() - mouseX)/ canvas.width,
                -2*(event.clientY.toFloat() - mouseY)/ canvas.height
        )
  ...
  fun update() {
    scene.update(gl, keysPressed, mouseDown)
...
fun main() {
  ...
  overlay.innerHTML = """<font color="blue">WebGL</font>"""
